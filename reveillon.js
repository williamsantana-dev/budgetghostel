/**
 * Gerador de Orçamento Réveillon - Pousada v2.1
...
calculateTotalPrice(roomId, checkinDate, checkoutDate) {
    ...
    let hasReveillonPeriod = false;
    
    // Verifica se há a diária do dia 31/12 no período
    const reveillonDate = new Date('2025-12-31');
    let hasReveillonNight = false;
    
    let currentDate = new Date(checkinDate);
    while (currentDate < checkoutDate) {
        if (currentDate.getTime() === reveillonDate.getTime()) {
            hasReveillonNight = true;
            break;
        }
        currentDate.setDate(currentDate.getDate() + 1);
    }
    
    if (hasReveillonNight) {
        // SEMPRE usar pacote de réveillon quando há a diária do 31/12
        hasReveillonPeriod = true;
        const reveillonPrices = this.reveillonPrices[roomId];
        
        if (totalNights <= 5 && reveillonPrices[totalNights]) {
            // Usar pacote específico (3, 4 ou 5 diárias)
            totalPrice = reveillonPrices[totalNights];
            if (totalNights > 3) {
                originalDailyRate = reveillonPrices[3] / 3;
            }
        } else if (totalNights > 5) {
            // Mais de 5 diárias: usar pacote de 5d + diárias extras
            totalPrice = reveillonPrices[5];
            originalDailyRate = reveillonPrices[3] / 3;
            
            const extraNights = totalNights - 5;
            const reveillonStart = new Date('2025-12-29');
            const reveillonEnd = new Date('2026-01-04');
            
            // Contar diárias extras antes e depois do réveillon
            currentDate = new Date(checkinDate);
            let extraPreNights = 0;
            let extraPostNights = 0;
            ...
            // Se temos mais de 5 diárias no período do réveillon, usar diária média do pacote 5d
            if (reveillonNights > 5) {
                const dailyRate5d = reveillonPrices[5] / 5;
                totalPrice = reveillonNights * dailyRate5d;
            }
            
            // Adicionar diárias pré e pós réveillon
            if (extraPreNights > 0) {
                totalPrice += this.preReveillonDailyRates[roomId] * extraPreNights;
            }
            if (extraPostNights > 0) {
                totalPrice += this.postReveillonDailyRates[roomId] * extraPostNights;
            }
        }
    } else {
        // Sem diária do 31/12 - usar valores normais
        const reveillonStart = new Date('2025-12-29');
        
        if (checkoutDate <= reveillonStart) {
            // Período totalmente antes do réveillon
            totalPrice = this.preReveillonDailyRates[roomId] * totalNights;
        } else if (checkinDate >= new Date('2026-01-04')) {
            // Período totalmente após o réveillon
            totalPrice = this.postReveillonDailyRates[roomId] * totalNights;
        } else {
            // Período misto sem o 31/12 (casos raros)
            currentDate = new Date(checkinDate);
            while (currentDate < checkoutDate) {
                if (currentDate < reveillonStart) {
                    totalPrice += this.preReveillonDailyRates[roomId];
                } else {
                    totalPrice += this.postReveillonDailyRates[roomId];
                }
                currentDate.setDate(currentDate.getDate() + 1);
            }
        }
    ...
    return {
        ...
        breakdown: {
            hasReveillonNight: hasReveillonNight,
